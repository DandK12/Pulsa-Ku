        ...
        <main className="flex-1 p-6">
          <div className="max-w-7xl mx-auto space-y-6">
            <div>
              ...
              <p className="text-gray-500 mt-1">Selamat datang kembali, {user?.name}! Berikut ringkasan hari ini.</p>
            </div>
                ...
                <div className="space-y-4">
                  {TOP_EMONEY.map((emoney, index) => (
                    <div key={emoney.id} className="flex items-center gap-4">
                      <div className="w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center text-xs font-bold text-gray-600">{index + 1}</div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input type="tel" placeholder="Contoh: 08123456789" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">Pilih Provider</label>
                    <div className="grid grid-cols-2 gap-2">
                      {PROVIDERS.map((p) => (
                        <button key={p.id} onClick={() => setSelectedProvider(p.name)} className={"py-2 px-3 rounded-lg text-xs font-semibold transition-all " + (selectedProvider === p.name ? p.color + ' text-white ring-2 ring-offset-1 ring-blue-400' : p.color + ' text-white opacity-70 hover:opacity-100')}>
                          {p.name}
                        </button>
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {DENOMINATIONS.map((d) => (
                        <button key={d.value} onClick={() => setSelectedDenom(d)} className={"py-2 px-3 rounded-lg text-left transition-all border-2 " + (selectedDenom?.value === d.value ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-blue-300')}>
                  ...
                  <button onClick={handleTambahKeranjang} className="w-full bg-blue-600 text-white py-3 rounded-xl font-semibold hover:bg-blue-700 transition-colors text-sm">
                    Tambah ke Keranjang
                  </button>
                </div>
              </div>
                  ...
                  return (
                    <div key={tx.id} className="flex items-center justify-between py-3 border-b border-gray-50 last:border-0">
                      ...
                      <div className="text-right">
                        ...
                        <span className={"inline-block px-2.5 py-0.5 text-xs rounded-full font-medium " + status.class}>{status.label}</span>
                      </div>
                    </div>
                  )
                })}
